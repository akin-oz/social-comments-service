...
The current advisory set is entirely in the `vitest`/`vite`/`esbuild` development chain — including one critical-rated advisory against the Vitest UI (`GHSA-5xrq-8626-4rwp`, arbitrary file access via the UI's dev server). None of it reaches the production image: `pnpm audit --prod` reports no known vulnerabilities, the runtime image installs with `--prod` so the whole dev chain is absent, and the Vitest UI server is never started — CI runs `vitest run`, not the UI. They are recorded rather than bumped, because bumping a test runner on the eve of a submission trades a real regression risk for a theoretical one that does not reach the deployed artefact. Re-check with `pnpm audit` (full tree) and `pnpm audit --prod` (runtime only).
...
Mind which connection string each context reads, because the variable name is deliberately overloaded and the trap is a copy-paste. `DATABASE_URL` is the **owner** connection when migrating and seeding (`pnpm migrate`, `pnpm seed`, and the CI job that runs them), and it is the **service** connection at runtime (`src/server.ts`). The test and CI harnesses keep the two apart by name — `DATABASE_URL` for the owner, `APP_DATABASE_URL` for the least-privileged `comments_app` role the service-under-test uses. Copying a migration-context `DATABASE_URL` (owner, and a superuser locally) into a running deployment is exactly the shared-credential mistake above: it starts, serves, and passes its health check with every tenant boundary quietly off. Point the runtime `DATABASE_URL` at `comments_app`; the Compose stack already does (`docker-compose.yml`), and the isolation tests prove the property holds only for that role.

...
- [ ] **The provider port has no room for a credential.** Specified as [Spec-016](../specs/016-provider-authorization-context.md). One adapter instance per platform is shared across tenants, and `PublishedPost` carries no social account, so A-002's authorised provider context has nowhere to go. A field on two interfaces today; a change to a widely depended-on domain type later.

...
Derivation assumes provider comment identifiers are unique within a platform.

**That assumption is weaker than this ADR originally claimed.** It asserted that every modelled provider satisfies it. Reading the vendor documentation shows only X states that its identifiers are globally unique; YouTube, Facebook, and Instagram do not document their uniqueness scope at all, and LinkedIn explicitly warns that the composite URN it returns "is not a reliable identifier", naming `(object, id)` as the dependable key. See the [capability matrix](../provider-capability-matrix.md) for citations.

The database is already stricter than the derivation: `unique (social_account_id, external_comment_id)` scopes provider identity per connected account, so the schema tolerates an identifier that repeats across accounts while the derived UUID does not. Folding the social account into the derivation would close the gap, at the cost of changing every existing identifier. That is a decision for a new ADR, to be taken before a live adapter is written rather than after.

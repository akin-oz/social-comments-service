  ...
  properties: {
    syncedAt: {
      ...
      description:
        ...
        'IMPORTANT: if this is null on any page of a pagination run, the run is reading a snapshot that is still being filled, and the list you assemble from it may be missing comments. Discard the pages you have, drop your cursor, and start the run again from the first page — a later run reads a completed snapshot and returns the full list. Continuing to follow nextCursor is safe but yields a partial view: no duplicates and no gaps in what it returns, and no indication that anything is missing.',
    },
  },
} as const;

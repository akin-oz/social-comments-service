...
> Interactive API docs (Swagger UI) at [`/documentation`](https://comments.akinoztorun.dev/documentation); the OpenAPI 3.1 document at [`/openapi.json`](https://comments.akinoztorun.dev/openapi.json). The two demo tenants are seeded there, so every example request below works against it by swapping `http://localhost:3000` for `https://comments.akinoztorun.dev`. Running on Fly.io with a PostgreSQL cluster; the service connects as a least-privileged, non-superuser role, so the row-level tenant isolation is enforced in the deployment, not only in tests. One always-on machine, so responses are immediate.
>
...
├── src/
│   ├── index.ts          # composition root: builds the app, decides the wiring
│   ├── server.ts         # the process — the only file that listens and exits
│   ├── migrate.ts        # the one migration runner, per release (ADR-0009)
│   ├── seed.ts           # the two demo tenants the examples below use
│   ├── openapi.ts        # writes docs/openapi.json from the route schemas
...
├── .github/              # CI workflow
├── .githooks/            # commit-msg gate, activated by `pnpm install`
└── scripts/              # the ai:sync wrapper, the commit-message check
```
...
CI runs the full sequence on every push to `main` — `install → ai:validate → generated-artefact drift gate → typecheck → lint → format:check → migrate → seed → test → openapi drift gate → build` — against a PostgreSQL service container, so the integration suite runs rather than skips. Two further jobs run alongside it: one checks the message of every commit the push adds, and one builds the image and brings `docker-compose.yml` up, then walks the documented requests against it, so both entry points this README offers are exercised rather than only the pnpm one. The latest run on `main` is green. (Two mid-history runs briefly showed red for reasons outside the repository — one `prettier --check` on a file the next commit added to `.prettierignore`, one runner it never acquired during a [declared GitHub Actions outage](https://www.githubstatus.com/) — both since superseded by green runs.)
...
A third sweep ran last, on the finished repository. Two of its five investigators worked by execution rather than reading: one stood up a scratch PostgreSQL container and deleted the tenant predicates itself, the other applied seventy mutations to a scratch copy and re-ran the suite after each. Forty-five of the seventy turned the suite red. What survived is closed and listed in [docs/tasks.md](docs/tasks.md) — the entry point that nothing executed, a retry budget asserted with checks that could not fail, an idempotency digest with no golden vector, and a fail-closed database guarantee claimed in prose and pinned by no test. Its most useful finding was about the gate rather than the code: the commit-trailer rule was enforced only inside an assistant session, so two commits typed in a terminal bypassed it, and one of them broke the build. That rule now runs as a git hook and a required CI job. What is still open — starting with the unauthenticated `X-Account-Id` header — is listed there too, with the reason it is open.

A fourth board — `principal-review`, five reviewers judging the design rather than its compliance — ran last and found the most consequential defect in the repository. The reply path splits its retry policy so that a write is never replayed, because a timeout does not prove the provider refused; but the delay logic answered for rate limits _before_ consulting that split, so the write policy's refusal was unreachable for a 429 and a publish went out three times for one idempotency key. Compounding it, a rate limit was recorded as a proven refusal, which answers the client's retry with _use a new key_ — publishing a fourth. Both are fixed under [ADR-0015](docs/decisions/0015-rate-limit-does-not-prove-refusal.md): a 429 from a limiter, a CDN, or a gateway can arrive after the origin already published, so nothing observable on the reply path proves refusal any more.

Worth stating plainly, because it is the more interesting half: this was not a team failing to see the distinction. `unknown` exists as a state precisely _because_ "the provider said no" and "the provider said nothing" are different, and the whole state machine, migration, and runbook were built around that line. The line was drawn correctly in one place and crossed in another. That is why the fix is a few lines rather than a redesign — and why no test caught it: every rate-limit test used a `Retry-After` above the retry budget, which takes the branch that does not replay, so the suite agreed the path was safe while it duplicated.

The board's P0 and P1 findings are closed, with the exceptions listed in [docs/tasks.md](docs/tasks.md) — extracting hydration from the read path, branded identifiers, a set-based upsert, and the cursor-rejection livelock are open and stated as open. One process finding is worth its own line: the automated half of the spec gate had never gated anything. It asked whether _any_ specification in the repository was approved rather than whether one covered the change, and all twenty-five were, so it passed unconditionally. Specifications now declare the paths they claim and the hook evaluates the claim ([Spec-032](specs/032-spec-gate-path-claims.md)).

> **One honest process note.** These fixes were implemented ahead of the spec gate rather than behind it, at the maintainer's explicit direction, with the specs and ADRs written alongside the code as records rather than approvals. That is a deliberate exception to this repository's own headline rule, it is noted in each document it touches, and the commit history shows it.
